...
$meters2 = 'ec3b0853-a3fa-5312-8425-f50fb1a33662','9d76fabc-1d5c-5208-8fbd-da65050e30a4','006be79b-30e2-5341-95b6-8c8bde9d7a04','de3493d9-ee03-5785-acf3-8d01b93a4a07','3b5e61ab-7f4d-5d88-8ad8-95fb9b2f1dfe'

# Get list price from Azure Pricing API for the specified meters
$meterFilter = ($meters | ForEach-Object { "meterId eq '$_'" }) -join ' or '
$meter2Filter = ($meters2 | ForEach-Object { "meterId eq '$_'" }) -join ' or '
...
$uri2 = "https://prices.azure.com/api/retail/prices?api-version=2023-01-01-preview&currencyCode='USD'&`$filter=($meter2Filter) and (armRegionName eq 'eastus2' or armRegionName eq 'Global')"
...
try {
    ...
    while ($uri2) {
        $response = Invoke-RestMethod -Method Get -Uri $uri2 -ErrorAction Stop
        if ($response.Items) {
            $retailPrices += $response.Items
        }
        # Follow pagination until NextPageLink is empty
        $uri2 = $response.NextPageLink
    }
} catch {
    Write-Error "Failed to retrieve retail prices from the Azure Retail Prices API. Error: $_"
}

# Calculate the monthly price for each retail price based on its unit of measure
ForEach ($retailPrice in $retailPrices) {
    switch ($retailPrice.unitOfMeasure) {
        ...
        '1K' { $retailPrice  | add-member -MemberType NoteProperty -Name monthlyPrice -Value $retailPrice.unitPrice }
    }
}
